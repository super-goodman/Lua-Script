...
function drink() --吃喝
    prints("a:62吃喝")
    local drinkBag, drinkSlot = getBelongBagAndSlotByID(8079)
    ...
    while getRoleCurrentHP() ~= -1 and getRoleflag1() ~= 0x04 do
        if IsBuffExist(29073) == 0 or IsBuffExist(22734) == 0 then
            useItem("魔法肉桂面包")
            useItem("魔法晶水")
        end
...
function move(x,y,z) --改写过的移动
    printf("a:62改写过的移动 %f,%f,%f",x,y,z)
    local x1,y1,z1 = getRolePosition()
    local n = 0
    while IsBuffExist(8326) == 0 and getDistanceBetween(x1,y1,x,y) > 0.8 and getRoleCurrentHP() ~= 0 and getRoleflag1() ~= 0x04  do
        moveB(x,y,z)
...
function GoBack(x,y)
    local roleX,roleY,roleZ,mapDistance1 = 0,0,0,0
    prints("a:62开场装备检测回城")
    useItem("炉石")
    sleep(19000)
    ...
    mapDistance1 = getDistanceBetween(roleX,roleY,x,y)--如果在炉石
...
function waitForGoBack()
    prints("a:62特殊等待炉石回城")
    local roleX,roleY,roleZ,mapDistance1 = 0,0,0,0
    local headDuration = getEquipementDurationBySlotNum(1)
    if headDuration <= 5 then
        useItem("炉石")
        sleep(19000)
        waitForLoading()
        sleep(5000)
        roleX,roleY,roleZ = getRolePosition()
        mapDistance1 = getDistanceBetween(roleX,roleY,124.939,-116.089)--如果在炉石
        if mapDistance1 <= 50 and mapDistance1 >= 0 then --如果在炉石 then
            return 1
        else
            return 0
...
function mainF()
    ...
    if IsBuffExist(8326) == 0 then -- 如果身上没有带着灵魂状态的Debuff
        ...
        if mapDistance1 <= 100 and mapDistance1 >= 0 then --如果在炉石 then
            ...
            castSpellA("冰甲术(等级 4)")
            castSpellA("魔法抑制(等级 5)")
            --useLua("/5.寻求组队 炽蓝仙野 固/定/今/团/公会/P1稳定FARM!欢迎各路爆炸治疗!输出SS/LR/元素/咕咕!欢迎小团体大团体加入做大做强再创辉煌!备战P2固定团来稳定选手")
            coolDown = getSpellCoolDown(11958)
            while coolDown == 0 and coolDown ~= -1 and getRoleCurrentHP() ~= -1 and getRoleflag1() ~= 0x04 do
                --prints("等待中")
                coolDown = getSpellCoolDown(11958)
                sleep(1000)
            end
            ...
            if shiledState == 1 and getRoleLevel() < 65 then
                moveB(0,0,0)
                sleep(8000)
            end
            castSpellA("闪现术")
            ...
            for i = 1, 50, 1 do
                ...
                if  flag1 == 0x08 then --在战斗中就循环战斗
                    if shieldState == 1 and getRoleLevel() < 65 then
                        moveB(0,0,0)
                        sleep(9000)
                        -- body
                    end
