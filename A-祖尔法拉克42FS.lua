...
--需要销毁的垃圾数量
mustDestroyItemNum = 5
--需要销毁的物品的ID
mustDestroyItem = {[1]=1520,[2]=1645,[3]=4421,[4]=4599,[5]=1520}
...
function attraction()
    ...
    while getGlobalFlag() == 1 and (getThreadTime() - time) <= attractionTime do
        if count >= attractionCount then
            break
        end
        local _, _, _, _, _, _, _, _,_, _, _, _, _, _, _,target = getClosetTargetIDFilterName("巫医",3)
        if target == 0 then
            count = count + 1
            printd(string.format("开始吸引墓地亡灵:%d,还剩时间:%d",count,attractionTime-(getThreadTime() - time)))
...
function main()
    if IsBuffExist(8326) == 0 then -- 如果身上没有带着灵魂状态的Debuff
        ...
        sleep(1200)
        --(第二个雪)
        storm(1,1883.11,1032.73,8.87802)
        sleep(1200)
        jumpB()
        --等待一会.不能动，因为怪也会卡住
        sleep(9000)
