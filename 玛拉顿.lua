    ...
    while getRoleCurrentMP() ~= -1 and getRoleCurrentMP() ~= 0 and getRoleflag1() ~= 0x04 do
        ...
        if distance >= 0 and ID1 ~= 0 and ID1 ~= -1 then
            if distance <= 9 and distance >= 0 and ID1 ~= 0 and ID1 ~= -1 then
                ...
                for i = 1, 50 do
                    spellID = getRoleCurrentSpellID()
                    if spellID ~= 0xCB and spellID ~= 0xCD and  spellID ~= 0x0a and i>= 5 and getRoleCurrentMP() > 40 then
                        --prints("暴风雪被打断")
                        castSpellA("暴风雪(等级 7)")
                        sleep(5)
                        castSpellPosition(x,y,z)
                    elseif spellID ~= 0xCB and spellID ~= 0xCD and spellID ~= 0x0a and i>= 5 and getRoleCurrentMP() <= 40 then
                        --prints("暴风雪被打断")
                        castSpellA("暴风雪(等级 1)")
                        sleep(5)
                        castSpellPosition(x,y,z)
...
function main()
    ...
    if IsBuffExist(8326) == 0 then -- 如果身上没有带着灵魂状态的Debuff
        ...
        if unusedBag >= 20 and headDuration >= 10 then --如果装备不红，背包没满，就出本继续
            -- 4. 出本流程(加延迟等待读条)
            -- 5. 重置副本
            -- 6. 这样构成一个循环
        ...
        elseif unusedBag <= 19 or headDuration <= 10 then --如果装备红了或者背包满了回城修理
