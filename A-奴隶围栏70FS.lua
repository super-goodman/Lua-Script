...
function iceBlock() -- 寒冰屏障
    ...
    while getRoleCurrentMP() ~= -1 and getRoleflag1() == 0x08 do
        ...
        --解散出去拉怪的水宝宝
        ID1,ID2 = getClosetTargetIDToAPositionFilterName("盘牙技师",21.7808,-370.019,3)
...
function summonStopMinion1(x,y,z)--水元素冻住怪1
    prints("a:水元素冻住怪1")
    castSpellA("召唤水元素")
    sleep(5)
    castSpellB("急速冷却")
...
function summonStopMinion2()--水元素冻住怪2
    prints("a:水元素冻住怪2")
    castSpellA("召唤水元素")
    sleep(600)
    castSpellB("冰冻术")
...
function move(x,y,z) --改写过的移动
    printf("a:改写过的移动 %f,%f,%f",x,y,z)
    local x1,y1,z1 = getRolePosition()
    local n = 0
    while getDistanceBetween(x1,y1,x,y) > 0.8 and getRoleCurrentMP() ~= -1 and getRoleflag1() ~= 0x04 and IsBuffExist(8326) ~= 1 do
...
function skip() --跳过下台子坐标
    local ID1,ID2 = getClosetTargetIDFilterName("盘牙卫士|沼泽龙虾人",3,8)
    local distance = getTargetAndRoleDistanceByID(ID1,ID2,3)
    printf("a:跳过下台子坐标距离 = %f",distance)
...
function attactTargetBeforeBoss() -- 攻击老1前的那波怪
    prints("尝试去拉BOSS前的小怪")
    ID1,ID2 = getClosetTargetIDToAPositionFilterName("盘牙技师",21.7808,-370.019,3)
    chooseTarget(ID1,ID2)
    useLua("/petattack target")
end
--主程序开始
--从本外开始
--useLua("/removefriend 炒鸡感激")
--useLua("/unignore 炒鸡感激")
