...
function findEnemy()
    ...
    ID1,ID2,x,y = getClosetTargetIDFilterNameNoDead("蟹|瞎",3)
    rotateA(x,y)
...
function waitForGetRidOfBattle()
    ...
    while getRoleflag1() == 0x08 and getGlobalFlag() == 1 and (getThreadTime() - time) < 80 do
        ...
        ID1,ID2,x,y = getClosetTargetIDFilterInBattle("蟹|瞎",3)
        rotateA(x,y)
...
function main()
    prints("开始西部荒野挂机,千万记得提前做好5组吃喝！！！-10935.3,2109.73,3.7723")
    prints("只有一个地点才可以刷！！！-10935.3,2109.73,3.7723")
    for i = 1, 100, 1 do
        if getGlobalFlag() ~= 1 then
            return
        end
